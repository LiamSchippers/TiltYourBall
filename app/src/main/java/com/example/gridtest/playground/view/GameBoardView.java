...
public abstract class GameBoardView extends View implements Observer {
	...
	public ArrayList<BlockCoordinate> blocks = new ArrayList<>();
	...
	public void onDraw(Canvas canvas) {
		...
		for (int x = 0; x < tileCountX; x++) {
			for (int y = 0; y < tileCountY; y++) {
				...
				int xStart = (mTileSize * x) + borderSizeX;
				int yStart = (mTileSize * y) + borderSizeY;
				int xEnd = (mTileSize * x) + mTileSize + borderSizeX;
				int yEnd = (mTileSize * y) + mTileSize + borderSizeY;
				...
				try {
					...
					if (object.getImageId().equals(WallObject.IMAGE_ID_WALL)) {
						index = 1;
					}
					if (object.getImageId().equals(HoleObject.IMAGE_ID_HOLE)) {
						index = 2;
					}
					if (object.getImageId().equals(FinishObject.IMAGE_ID_FINISH)) {
						index = 3;
					}
					if (object.getImageId().equals(TeleportFromObject.IMAGE_ID_TELEPORT_FROM)) {
						index = 4;
					}
					if (object.getImageId().equals(TeleportToObject.IMAGE_ID_TELEPORT_TO)) {
						index = 5;
					}
				...
				blocks.add(new BlockCoordinate(xStart, xEnd, yStart, yEnd,index));
